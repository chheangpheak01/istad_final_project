import DataTable from "react-data-table-component";
import ActionButtons from "./ActionButtons";

export default function MovieTable({
  data,
  windowWidth,
  onWatch,
  onSave,
  onDelete,
  onRestore,
  onOpenMovieDetail,
  isLoading,
  activeTab,
  onLoadMore,
  emptyMessage = "🎬 No movies found for your search.",
}) {
  const columns = [
    {
      name: "Poster",
      selector: (row) => row.poster_path,
      cell: (row) => (
        <img
          src={
            row.poster_path
              ? `https://image.tmdb.org/t/p/w200${row.poster_path}`
              : "https://via.placeholder.com/200x300?text=No+Image"
          }
          alt={row.title}
          className="w-12 h-16 sm:w-16 sm:h-24 md:w-20 md:h-28 object-cover rounded-lg shadow-md"
        />
      ),
      grow: 0,
    },
    ...(windowWidth >= 640
      ? [
          {
            name: "Title",
            selector: (row) => row.title,
            sortable: true,
            cell: (row) => (
              <span className="font-semibold text-gray-800 text-sm sm:text-base">
                {row.title}
              </span>
            ),
          },
        ]
      : []),
    ...(windowWidth >= 768
      ? [
          {
            name: "Release Date",
            selector: (row) => row.release_date,
            sortable: true,
            cell: (row) => (
              <span className="text-gray-600 text-xs sm:text-sm">{row.release_date}</span>
            ),
          },
        ]
      : []),
    {
      name: "Action",
      cell: (row, index) => (
        <ActionButtons
          key={`${row.id}-${index}`}
          row={row}
          onWatch={onWatch}
          onSave={onSave}
          onDelete={onDelete}
          onRestore={onRestore}
          onOpenMovieDetail={onOpenMovieDetail}
          activeTab={activeTab}
        />
      ),
    },
  ];

  return (
    <section
      className="bg-white rounded-2xl shadow-lg overflow-hidden border border-amber-100 w-full"
      aria-label="Movies Table"
    >
      <div className="overflow-x-auto">
        {isLoading ? (
          <header className="flex justify-center items-center py-10" aria-live="polite">
            <div className="flex items-center space-x-3 text-amber-600">
              <span
                className="loader border-4 border-amber-200 border-t-amber-600 rounded-full w-8 h-8 animate-spin"
                aria-hidden="true"
              ></span>
              <span className="font-medium text-lg">Loading movies...</span>
            </div>
          </header>
        ) : data.length === 0 ? (
          <header className="text-center py-10 text-gray-500 font-medium" aria-live="polite">
            {emptyMessage}
          </header>
        ) : (
          <>
            <DataTable
              columns={columns}
              data={data}
              pagination
              highlightOnHover
              striped
              className="w-full"
              responsive
              keyField={(row, index) => `${row.id}-${index}`}
            />

            {activeTab === "loadMore" && onLoadMore && (
              <div className="flex justify-center py-4">
                <button
                  className="bg-amber-500 text-white px-4 sm:px-6 py-2 rounded-md hover:bg-amber-600 transition-all duration-300 text-sm sm:text-base"
                  onClick={onLoadMore}
                >
                  Load More
                </button>
              </div>
            )}
          </>
        )}
      </div>
    </section>
  );
}

